    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->has('categories')) {
                // تحقق مما إذا كانت الفئات مصفوفة أو سلسلة مفصولة بفواصل
                $categories = $request->categories;
                if (!is_array($categories) && strpos($categories, ',') !== false) {
                    $categories = explode(',', $categories);
                }

                if (is_array($categories)) {
                    $filteredCategories = array_filter($categories); // إزالة القيم الفارغة
                    ...
                    \Illuminate\Support\Facades\Log::info('Filtering by single category', [$categories]);
                }
            }
            ...
            if ($request->filled('level')) {
                ...
                \Illuminate\Support\Facades\Log::info('Filtering by level', [$request->level]);
            }
            ...
            if ($request->filled('price_range')) {
                ...
                \Illuminate\Support\Facades\Log::info('Filtering by price range', [$priceRange]);
            }
