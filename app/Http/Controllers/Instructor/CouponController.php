...
{
    /**
     * Display a listing of the instructor's coupons.
     *
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'courses_applicable' => 'required|array',
        ...
        // Ensure the instructor only applies coupons to their own courses
        $instructorCourseIds = Course::where('instructor_id', Auth::id())
            ->pluck('course_id')
            ->toArray();
        
        $coursesApplicable = array_intersect($request->courses_applicable, $instructorCourseIds);
        
        if (empty($coursesApplicable)) {
            return back()
                ->withErrors(['courses_applicable' => 'You must select at least one of your own courses.'])
                ->withInput();
        }
