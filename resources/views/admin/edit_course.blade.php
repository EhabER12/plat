...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1>تعديل الكورس</h1>
            <div>
                <a href="{{ route('admin.courses.show', $course->course_id) }}" class="btn btn-info me-2">
            ...
            <div class="card-body">
                <form action="{{ route('admin.courses.update', $course->course_id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="instructor_id" name="instructor_id" required>
                                    ...
                                    @foreach($instructors as $instructor)
                                        <option value="{{ $instructor->user_id }}" {{ (old('instructor_id', $course->instructor_id) == $instructor->user_id) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" class="form-control" id="duration" name="duration" value="{{ old('duration', $course->duration) }}" min="1">
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="language" name="language" required>
                                    <option value="العربية" {{ old('language', $course->language) == 'العربية' ? 'selected' : '' }}>العربية</option>
                                    <option value="English" {{ old('language', $course->language) == 'English' ? 'selected' : '' }}>English</option>
                                    <option value="Español" {{ old('language', $course->language) == 'Español' ? 'selected' : '' }}>Español</option>
                                    <option value="Français" {{ old('language', $course->language) == 'Français' ? 'selected' : '' }}>Français</option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="approval_status" name="approval_status" required>
                                    <option value="pending" {{ old('approval_status', $course->approval_status) == 'pending' ? 'selected' : '' }}>قيد المراجعة</option>
                                    <option value="approved" {{ old('approval_status', $course->approval_status) == 'approved' ? 'selected' : '' }}>تمت الموافقة</option>
                                    <option value="rejected" {{ old('approval_status', $course->approval_status) == 'rejected' ? 'selected' : '' }}>مرفوض</option>
                                </select>
                            </div>
                            
                            <div class="mb-3 form-check">
                                <input type="checkbox" class="form-check-input" id="featured" name="featured" value="1" {{ old('featured', $course->featured) ? 'checked' : '' }}>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                        <a href="{{ route('admin.courses') }}" class="btn btn-secondary me-md-2">إلغاء</a>
